...
/// Controller that exposes the current height selection state to a parent
/// widget.  Create one instance per [StartHeightPicker] and pass it in; listen
/// to it (e.g. with [ListenableBuilder]) to react to changes.
class StartHeightPickerController extends ChangeNotifier {
  bool _isUsingDate = true;
  int _height = 0;
  bool _hasBlockHeight = false;

  /// The currently computed block height (0 when using date with no date
  /// chosen, or when no block-height text has been entered).
  int get height => _height;

  /// Whether the picker is in date mode.
  bool get isUsingDate => _isUsingDate;

  /// Whether the user has entered a block height value in block-height mode.
  bool get hasBlockHeight => _hasBlockHeight;

  /// Whether the current state satisfies the minimum requirement to proceed
  /// (either date mode is active, or a block height has been typed in).
...
/// a raw block height as the starting point for a wallet scan or restore.
///
/// All internal state is managed here; the parent receives updates through
